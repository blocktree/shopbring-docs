...
- **信用增值模型**。为了刺激更多用户在Shopbring经常购物，我们建立起信用成长制度，每个时期会以信用值占比来空投代币奖励。正常完成代购交易会增加双方信用值，相反欺诈行为会减少信用值，参与纠纷仲裁的仲裁者会增加信用值。
...
为了激励平台用户增加信用值，系统在每个周期会根据整体用户平均信用值等级确定空投代币数量，平均信用值越高，空投代币总数越大。用户所得空投代币数量 = 单个信用值空投代币数 * 用户信用值。
...
在退货过程中难免会产生一些纠纷，交易双方都可提交申诉，让仲裁者介入。仲裁者一般根据商品所属的电商平台服务条款来确定仲裁结果，有如下两种情况：
...
> 因为暂时不知道平行链插槽拍卖的具体日期，所以主网上线时间不确定。我们会首先寻求在Kusama上租用平行链插槽，实现Shopbring的用户增长目标。
